-- Wallet-Model schema for Cloudflare D1
-- Tokens never expire. Every PAID transaction mints tokens proportionally.
-- Apply with: wrangler d1 execute <DB_NAME> --file=./schema/wallet-schema.sql
...
-- 2) Plan catalog (align plan_id with your Clerk plan/price id)
...
-- 3) Wallet balance per subject (user or organization)
...
-- 4) Wallet ledger (auditable history, also used for idempotency of mints)
...
-- 5) Entitlements (current plan for feature gating; balance is independent)
...
-- 5) Usage logs (detailed API usage tracking)
CREATE TABLE IF NOT EXISTS usage_logs (
  ...
  -- API details
  provider TEXT NOT NULL, -- 'openai', 'comet', 'gemini', etc.
  ...
  request_id TEXT,
  response_id TEXT,
  event_type TEXT, -- for realtime events
...
-- Migration helper views (optional, for monitoring)
