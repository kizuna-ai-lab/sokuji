...
export interface SonioxCloneReviewStepProps {
  /** The picked/recorded clip, played back so the user can check it before
   *  it's uploaded. Never null: the caller mounts this only once a clip is
   *  staged, so there is no closed state to represent. */
  audioBlob: Blob | null;
  /** Mapped `create()` failure message (e.g. voice_name_conflict) — set by
   *  the caller to keep this step mounted for a rename-and-retry. */
  error: string | null;
...
/**
 * Post-acquisition review step for Soniox voice cloning: rendered once a clip
 * has been picked/recorded and has passed client-side validation, before it
 * is uploaded. A usage-rights checkbox ("I confirm I have the right to use
 ...
 * mapped create failure — keeps it mounted with `error` set so the user can
 * rename and retry without losing the clip.
 *
 ...
 * conflict and retry.
 */
const SonioxCloneReviewStep: React.FC<SonioxCloneReviewStepProps> = ({
  audioBlob,
  error,
  busy,
  notice,
...
export default SonioxCloneReviewStep;
