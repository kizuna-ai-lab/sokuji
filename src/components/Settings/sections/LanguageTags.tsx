 ...
 * 99-language ASR model no longer stacks rows of chips under its name.
...
export const LanguageTags: React.FC<{ languages: readonly string[] }> = ({ languages }) => {
  ...
  if (codes.length === 1) {
    const only = codes[0];
    return <span className="model-card__lang-tag">{MULTI_MARKERS.has(only) ? multiLabel : only}</span>;
  }
  ...
  return (
    <Tooltip content={content} position="top" maxWidth={320}>
      <span className="model-card__lang-tag model-card__lang-tag--multi">{multiLabel}</span>
    </Tooltip>
  );
};
