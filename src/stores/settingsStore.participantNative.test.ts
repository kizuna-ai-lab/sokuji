...
// Kept from before the old audio service was deleted: importing
// settingsStore.ts (directly, and via createParticipantLocalNativeConfig
// below) used to drag in its real static import graph — including
// audioStore -> ServiceFactory, which imported ModernBrowserAudioService ->
...
// graph for that reason. Mocked anyway (same fix modelStore.test.ts and
// nativeModelStore.test.ts already use) so settingsStore's own persistence
