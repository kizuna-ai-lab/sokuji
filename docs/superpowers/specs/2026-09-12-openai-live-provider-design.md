...
| Event | Handling |
| --- | --- |
| `session.input_transcript.delta` | Find the last sentence end inside `delta` (`lastSentenceEnd`, judged with the item's transcript so far as context). If the current item exists and `start_ms` is ≥ 600 ms past the previous delta's `end_ms`, complete it first (unless the delta's head is punctuation only, which belongs to that item). Append the head to the current item (`appendUserText`, opening one if needed, leading whitespace dropped on a fresh item), `onConversationUpdated({ item, delta: { transcript } })`; if a sentence end was found, `completeUserItem()`; append the remainder to a fresh item. A punctuation-only head with no item to attach to is dropped. Then, if the item now spans ≥ 12 s of timeline (`end_ms` − its first `start_ms`), complete it. Reset the user silence timer while an item is open. |
| `session.output_transcript.delta` | Same split on the assistant side (`appendAssistantText`, which also records the delta's `end_ms` for the item): the head goes to the current item and, if a sentence end was found or the item spans ≥ 15 s of timeline, the item's text is closed (`closeAssistantText`: it joins the pending-audio queue and later deltas open a new item); the remainder opens the next item. Reset the assistant silence timer. |
