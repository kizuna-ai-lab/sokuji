...
**Status:** design approved in conversation; second draft after adversarial review
(findings in `docs/superpowers/notes/2026-09-04-device-profile-review-findings.md`);
...
6. **The only capability question a backend answers truthfully is `supports_op`.** No
   ...
   each backend's real rule including its build flags and env overrides. Every
   capability decision in this spec is therefore an op-coverage decision; the raw
   feature bits in the profile are reported for diagnostics and never gate anything.
...
- **Native.** `sk_device` (`native/include/sokuji_native.h:88-95`) is
  ...
  backend-specific symbols (`ggml_backend_metal_supports_family`,
  `ggml_get_type_traits_cpu`) are not linkable from `libsokuji_native`; only what a
  registration exposes through `ggml_backend_reg_get_proc_address` is reachable, and
  for the CPU and Metal registrations that is `ggml_backend_get_features` alone.
  ...
  cross-checks: `#define SK_ABI_VERSION 1` (`sokuji_native.h:42`),
  ...
  `transcribe`, `llama`, `engine_runtime` (`CMakeLists.txt:52-81`). The Linux wheels
  allow a `libvulkan.so.1` DT_NEEDED on the dlopen'd ggml-vulkan module only
  (`native/ci/check_linux_deps.py`, docstring).
...
- **Sidecar.** `Machine` (`accel.py:20-45`) carries `tc_kinds` and
  ...
  `gpu_possible`) — and so do `_h_models_catalog` (`tiers[].available`),
  `_h_list_variants` and `_engine_identity` (`preferredDevice`); its `backend`
  parameter is accepted and unread. Bench entries are keyed by
  ...
  `~/.cache/sokuji-sidecar/accel-bench.json` by `bench_load`/`bench_save` (490-509)
  with a plain `open(path, "w")`. `_h_hardware_info` (731) returns `gpus[]` as
  `{vendor, name, vramMb}` plus the four `_engine_identity` fields;
...
- **Tests.** `test_planner.py:191-275` pins `_tier_available` (nine cases including the
  ...
  devs, *, version="1.0.1", engine_versions=None)` at 929; `test_characterization.py`
  builds its four `Machine`s directly with keyword arguments at import (lines 55-80),
  never through `probe()`, and pins only `_downloaded_quants` and `bench_load`.
...
```c
enum sk_feature {
    /* Vulkan: RAW extension/feature bits from the physical device. ggml applies further
     * gates before using any of them (build-time GGML_VULKAN_*_GLSLC_SUPPORT, the
     * GGML_VK_DISABLE_* environment, and per-driver deny-lists for coopmat), so a set
     * bit means "the device offers it", not "ggml uses it". Diagnostics only. */
    ...
    /* Metal: DERIVED from the backend's own supports_op, so they equal what ggml
     * will do (has_simdgroup_reduction / has_bfloat, including GGML_METAL_BF16_DISABLE
     * and the Metal4 dummy-compile fallback). Diagnostics only; §3.2 is the gate. */
    SK_FEAT_MTL_SIMDGROUP_REDUCTION = 1u << 5, /* supports_op(NORM[f32]) on the Metal device */
    SK_FEAT_MTL_BFLOAT              = 1u << 6, /* supports_op(MUL_MAT[bf16 x f32]) on the Metal device */
    /* Both: */
...
typedef struct sk_device_profile {
    ...
    char     driver_version[64];  /* Vulkan: driverInfo; Metal: sysctlbyname("kern.osversion"); CPU: "" */
    char     device_uuid[40];     /* Vulkan: VkPhysicalDeviceIDProperties.deviceUUID as 32 hex chars; else "" */
    char     cpu_features[256];   /* CPU device only: "AVX2=1,FMA=1,..." from ggml_backend_get_features; else "" */
} sk_device_profile;

/* SK_ERR_INVALID_ARGUMENT for a bad index or NULL out; SK_ERR_NOT_INITIALISED before
 * sk_init; otherwise SK_OK, with known = 0 when the profile could not be read. */
...
- **Vulkan.** The library enumerates physical devices itself, **without linking the
  ...
  `check_linux_deps.py` asserts that for `libsokuji_native.so` while keeping its
  allowance for the ggml-vulkan module. CMake: when `SOKUJI_GPU_RESOLVED` is
  `vulkan`, `find_package(Vulkan QUIET)` runs in native's own scope (it does not
  today when the lane is passed explicitly) and `sokuji_native` links
  `Vulkan::Headers` only. ggml's device `i` is matched to a physical device by
  **description string**, and among identical descriptions by order; two identical
  GPUs may swap profiles (§6). It never touches ggml-vulkan's private `vk_device`.
- **Metal.** The two capability bits are read through `ggml_backend_dev_supports_op`
  on the Metal device with one scratch tensor each — `NORM[f32]` for simdgroup
  reduction, `MUL_MAT[bf16 × f32]` for bfloat — because that predicate is ggml's own
  expression of `has_simdgroup_reduction` (Apple7 **or** Metal3) and `has_bfloat`
  (Apple6 / Metal3, minus `GGML_METAL_BF16_DISABLE` and the Metal4 fallback), and
...
on the device and no backend device is initialised by this call.

### 3.2 Native: `sk_device_supports_ops`

```c
typedef struct sk_op_check { char name[48]; int32_t supported; } sk_op_check;   /* "OP[src0,src1]", e.g. "MUL_MAT[q4_K,f32]" */
 ...
 * compute_type: the rung ("q4_k_m" | "q5_k_m" | "q6_k" | "q8_0" | "f16" | "bf16"), which
 * the implementation maps to the weight ggml_type substituted for WEIGHT in the table.
 * Unknown (stage, family) → SK_ERR_NOT_FOUND; unknown compute_type or bad index →
 * SK_ERR_INVALID_ARGUMENT; a backend exception → SK_ERR_BACKEND (caller treats as
...
(`no_alloc`) holding one representative tensor per entry — `(GGML_OP, src0 type,
src1 type)` — so the answer is the backend's own `supports_op` predicate, exactly the
check ggml's scheduler makes per node and the check llama.cpp makes per weight tensor
when it places buffers. Table entries whose weight dtype is the rung's use the
placeholder `WEIGHT`; the call substitutes the rung's `ggml_type` (`q4_k_m → Q4_K`,
`q5_k_m → Q5_K`, `q6_k → Q6_K`, `q8_0 → Q8_0`, `f16 → F16`, `bf16 → BF16`), so one
family table serves every rung and a device that refuses one dtype loses only that
rung.
...
(`SK_ERR_BACKEND` on exception), and it is placed where the same initialisation would
be paid anyway — in the `accel.resolve_*` wrappers immediately before the load that
follows them (§3.3) — never on the sidecar-ready critical path and never inside the
pure planner. On Metal and CPU the predicate is cheap. On no backend does the query
execute a graph, so it cannot `GGML_ABORT`.
...
reports after a load. Two things keep the tables honest:

- **A CTest that diffs them against the real graph** (`test_ops_coverage`). Op
  recording does not depend on `ggml_backend_sched` (audio.cpp has none): the test
  build wraps the CPU backend in a thin recording backend — a `ggml_backend_i` whose
  `graph_compute` walks `cgraph->nodes[i]`, records `(op, src[0]->type,
  src[1]->type)`, and forwards to the real CPU backend (`ggml-backend-impl.h` is
  already on native's include path via `audiocpp_compat.h`) — and hands that backend
  to each engine for one forward pass of every family with a cached test model
  (`~/.cache/sokuji-native-tests/`: whisper-tiny, moonshine-streaming-tiny,
  Qwen3-0.6B, moss-tts-nano, supertonic-3 today). llama.cpp is recorded with flash
  attention both on and off, since its graph differs by device. The recorded set must
  be ⊆ the table (after `WEIGHT` substitution); a pin bump that adds an op turns the
  test red naming the entry — the same discipline as the exact-text patches in
  `native/patches/`. Skip rule: return code 77 only when no family's model is present;
  otherwise the present families are asserted and each absent one prints
...
- **Recorded dumps for families the test cannot load.** The same recording backend,
  run through a `--dump-ops <stage> <family> <model-dir>` flag on the test binary,
  writes `native/src/ops/<stage>-<family>.ops` (one `OP[src0,src1]` per line, the
  engine pin on line 1). A table entry set is checked against its dump by a plain
  CTest; the checklist line covers regenerating dumps on a bump.
- **The tables are data, not code.** One `static const` array per family, names
  matching `ggml_op_name()` so a diff reads directly against ggml's log output.
...
catalog invariant test asserts `graph_family` is non-empty on every card.
...
class Machine:
    ...
    generation: str = ""                      # "" only when sokuji_native is absent
```
...
Two new detectors, module-level like the existing four so tests can patch them:
`_native_profiles() -> tuple[DeviceProfile, ...]` (one `sk_device_profile_get` per
`sk_devices()` entry, through a new `native.device_profiles()` wrapper) and
`_native_generation() -> str`. `probe()` wraps both in `_safe` — a raise yields
`devices=()` / `generation=""`. A sidecar running against a 1.0.x wheel therefore
**loads it and degrades**: `native.device_profiles()` fails on the missing symbol,
`_safe` returns `()`, `generation` is still computed from `sk_version` and
`engine_versions`, and every resolve is unchanged (premise 5).
...
generation = blake2s(
    ...
    f"{[(d.kind, d.device_uuid, d.driver_name, d.driver_version) for d in devices sorted by index]}|"
...
and never across one. The cache file gains a `"_generations": [...]` list (newest
last); `bench_save` appends the current generation on first save under it, prunes
every entry whose prefix is not among the last three, and writes through a temp file
plus `os.replace`; `bench_load` strips the list before returning entries. A legacy
flat file (no `_generations`) is generation-less: its keys can no longer match and it
is dropped at the first save.

**Op coverage cache.** `accel.op_coverage(machine, device_index, stage, family,
compute_type) -> OpCoverage | None` calls `native.device_supports_ops` once per
`(generation, device_index, stage, family, compute_type)` and stores
`{"allSupported": bool, "unsupported": ["NORM[f32,-]", ...]}` under key
`f"{machine.generation}|ops:{device_index}:{stage}:{family}:{compute_type}"` in the
same file. `SK_ERR_BACKEND` (the Vulkan first-init exception) is not cached and
returns `None`. **Who calls it:** the `accel.resolve` / `resolve_translate` /
`resolve_tts` wrappers, for the resolved card's `graph_family` and each
`compute_type` the card lists, on each non-cpu device whose profile is `known` — and
only then; with `devices=()` or `known=False` the wrappers never touch native. They
pass the results to the planner as an injected callable (the pattern `est_bytes` /
`format_ready` already use): `op_coverage: Callable[[int, str, str, str],
OpCoverage | None]`, keyword-only, threaded through the four public entry points
(`resolve`, `resolve_translate`, `resolve_tts`, `select_variant`). `_h_models_catalog`,
`_h_list_variants` and `_engine_identity` pass a **read-only** callable
(`cache.get`; miss → `None`) so the catalog never computes coverage on the
sidecar-ready path — a tier stays `available: true` until the first resolve of that
family fills the cache (premise 5; §6).
...
def _deployment_available(model, d: Deployment, machine: Machine, *, op_coverage) -> bool:
    ...
    stage = _STAGE_OF_BACKEND[d.backend]
    cov = op_coverage(dev.index, stage, model.graph_family, d.compute_type)
    if cov is None:
        return True                                   # not computed yet, or backend exception
...
`_tc_pick_quant`, and the three wire producers (`_h_models_catalog`'s
`tiers[].available` and `variants[].supported`, `_h_list_variants`,
`_engine_identity`'s `preferredDevice`). A pin, a downloaded rung and an explicit
`override="gpu"` therefore all resolve a TTS rung the device cannot execute to its
cpu row instead of aborting the process, and the catalog stops advertising that tier
for it. There is deliberately no feature-bit rule: Vulkan accepts `MUL_MAT[bf16]`
unconditionally (the extension only selects a faster kernel), so a bit-based bf16
gate would refuse rungs ggml runs — the table's `MUL_MAT[WEIGHT]` entry is the answer.
...
description match. Both existing paravirtual tests stay green; two new ones cover the
structured signal (known profile without the bit → refused; known profile with the bit
and a description containing "paravirtual" → still refused, because the string rule is
kept until every shipped wheel carries profiles).

### 3.4 Wire and renderer

`hardware_info_result` gains two **optional** fields, added to
...
`toEqual` test stays as is). When a stored profile lists unsupported ops for a
`"tts/…"` key not seen before this session, the store reports once —
`reportWarning('NativeModelStore', …, { dedupeKey: 'native.ops.unsupported' })`
guarded by a `reportedUnsupportedOps: Set<string>` on the store (`dedupeKey` alone
only throttles bursts) — so the op names reach LogsPanel. No new user-facing strings,
no new UI control: a profile is recomputed on every sidecar start and keyed by
(hardware, native version, driver), so a manual "re-probe" action has no scenario in
this spec and is left to a later one.
...
- **native (CTest).** `test_common.cpp`: `sk_device_profile_get` returns `SK_OK` on
  every device; the CPU profile has non-empty `cpu_features`; on a Vulkan build with
  a device, `known == 1`, `device_uuid` is 32 hex chars, `driver_name` non-empty; on
  Apple Silicon `SK_FEAT_MTL_SIMDGROUP_REDUCTION | SK_FEAT_MTL_BFLOAT | SK_FEAT_UMA`
  are set; bad index or NULL → `SK_ERR_INVALID_ARGUMENT`; before `sk_init` →
  `SK_ERR_NOT_INITIALISED`. `sk_device_supports_ops` for every `(stage, family)` in
  the tables and every rung returns `all_supported == 1` on the CPU device;
  `SK_ERR_NOT_FOUND` for an unknown pair; `SK_ERR_INVALID_ARGUMENT` for an unknown
  compute_type. `test_ops_coverage` and the dump checks as in §3.2. The
...
- **sidecar catalog.** Every card has a non-empty `graph_family`; `prompt_family`
  unchanged on every translate card; for each cached test model, `sk_asr_caps.arch`
  equals the card's `graph_family`.
- **sidecar planner.** `_deployment_available`: unknown profile → identical to
  `_tier_available`; known profile, `tts`, an unsupported op → False on gpu tiers,
  True on cpu; known profile, `translate` or `asr`, an unsupported op → True (runs
  with fallback); coverage `None` → True; the same TTS rung reaches the cpu row under
  `pin`, under `downloaded={that rung}`, and under `override="gpu"` (three tests, one
  per bypass path); a bf16 rung whose `MUL_MAT[bf16,f32]` is unsupported is refused
  while its `q8_0` sibling is kept (per-rung, not per-family). Structured paravirtual
  cases as in §3.3. Bench keys: an entry written under generation G is read back by
  `resolve` / `resolve_translate` under G and ignored under G′; the cache-building
  tests at `test_planner.py:395, 408, 512, 848` move to `_cache_key`. Existing
  `_tier_available` tests unchanged.
- **sidecar accel.** `probe()` fills `devices` from a `_fake_native_module` that grows
  ...
  `probe(force=True)` test patches `_native_profiles` and `_native_generation` too
  (a shared `_isolate_probe` fixture); `_native_profiles` raising → `devices=()`;
  `_native_generation` raising → `generation=""`; a fake module **without** the two
  new functions → `devices == ()`, `generation != ""`, and `resolve_tts` output
  identical to the no-profile machine; `generation` changes when `version`, an engine
  pin, a driver string or a `GGML_*` variable changes and not when only `mem_free`
  does; `_measure` misses on a generation change and hits within one
  (`test_accel.py:413, 430` move to `_cache_key`); `bench_save` is atomic and keeps
  exactly the last three generations (`_generations == [G2, G3, G4]` after saves
  under G1..G4); a legacy flat file is dropped on first save; `op_coverage` calls
  `native.device_supports_ops` once per key across two resolves and never when
  `devices=()` or `known=False`; `SK_ERR_BACKEND` is not cached; `_h_hardware_info`
  emits `devices[]` and never computes coverage; `_h_models_catalog` marks
  `gpu-vulkan` unavailable and the rung `supported: False` for a TTS card whose cached
  coverage is unsupported, and leaves both untouched on a cache miss.
- **characterisation.** The `Machine` defaults keep every matrix row unchanged; an
  autouse fixture in that file monkeypatches `accel.op_coverage` and `native.module`
  to `pytest.fail("native reached with devices=()")`, and one assertion checks
  `all(m.devices == () and m.generation == "" for m in _ALL_MACHINES)`.
- **renderer.** `nativeProtocol.consistency.test.ts` pins the two optional fields
  ...
  `engineInfo` is unchanged; the warning is reported once per `"tts/…"` key with the
  op names.

## 5. Out of scope

- Which rung is recommended, downloaded-wins, auto-pin on download, per-variant
  reason codes, the "exactly one `recommended`" guarantee, the smallest-rung renderer
  fallback — **spec B**.
...
- A user-triggered re-probe, and any use of the raw feature bits beyond diagnostics.
...
- **Vulkan's first coverage query costs seconds** (device creation + pipeline
  compile), paid in the resolve wrapper right before the load that would pay it
  anyway. On a cold driver cache the first load of a session is slower by that much
  than today; after that it is free within the process.
- **Identical GPUs.** Description-string matching can swap the profiles of two
  identical cards; only differs if their drivers differ.
- **Op tables drift with pins.** Mitigated by `test_ops_coverage` for families with
  cached models and by the dump checks plus the pin-bump checklist for the rest; a
  stale table fails closed on a *new* op only if the test runs — the checklist is a
  procedural gate, not an automatic one.
...
- 2026-09-04 — second draft after review (`docs/superpowers/notes/2026-09-04-device-profile-review-findings.md`):
  no feature-bit gate (Vulkan runs bf16 without the extension); the gate is per rung
  (`WEIGHT` substitution) and per stage (only audio.cpp aborts); Metal bits derived
  through `supports_op`, not `supports_family` (unreachable under `GGML_BACKEND_DL`);
  the Vulkan loader is dlopen'd, never linked; coverage is computed in the resolve
  wrappers, never on the ready path; `graph_family` added to every card; the
  re-probe action dropped (Claude).
