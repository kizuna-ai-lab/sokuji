...
| `src/components/Subtitle/SubtitleApp.tsx` | Add `surface?: 'electron' \| 'pip'` prop (default `'electron'`). When `surface === 'pip'`, skip the `subtitle:window-bounds-changed` listener. The ESC handler keeps using `window.addEventListener('keydown', …)`; when `SubtitleApp` is rendered inside the PiP window via the second React root, `window` refers to the PiP window and ESC works as expected. |
...
5. **ESC inside PiP** — `SubtitleApp`'s existing keydown listener fires (on the PiP window). It calls `exitSubtitleMode()` → `pipWindow.close()` → `pagehide`.
