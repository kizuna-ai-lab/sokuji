...
Sokuji 是一款跨平台实时语音翻译应用，同时支持桌面端和浏览器端。支持**本地推理** — 通过 WASM 和 WebGPU 在设备上运行 ASR、翻译和 TTS，无需 API 密钥，无需昂贵的 GPU，完全离线，隐私完全保护。同时集成 OpenAI、Google Gemini、Palabra.ai、Kizuna AI、豆包 AST 2.0、Soniox 和 OpenAI 兼容 API 等云端服务商。
...
| | |
|---|---|
| **服务商** | 8 — OpenAI、Gemini、Palabra.ai、Kizuna AI、豆包 AST 2.0、Soniox、OpenAI 兼容、本地推理 |
