// segment-any-text/sat-3l-sm (wtpsplit "Segment any Text", 3-layer XLM-R, "sm" variant) ported to
// onnxruntime-web. It predicts sentence boundaries and inserts no marks, so the output is the input
// with '\n' at each predicted boundary.
//
// A faithful port of wtpsplit 2.2.1 `SaT.split(text)` with default arguments:
...
//  - sigmoid in float32, boundary where p > 0.25 (wtpsplit's default for model names with "sm")
...
//
// Browser-compatible ESM: only the injected dependencies are used.

export const info = {
  id: 'sat-3l-sm',
  name: 'SaT sat-3l-sm (wtpsplit, XLM-R 3 layers, boundaries only)',
  langs: ['ja', 'zh', 'en', 'ko'],
  output: 'boundary',
  localDir: '/home/jiangzhuo/.cache/sokuji-punct-bench/sat/sat-3l-sm',
  files: ['model.onnx', 'tokenizer.json'],
};

export const DEFAULTS = { threshold: 0.25, blockSize: 512, stride: 64, batchSize: 32 };
...
const isPySpace = (ch) => PY_SPACE.has(ch.codePointAt(0));
...
const f16round = typeof Math.f16round === 'function' ? Math.f16round : (x) => {
...
function logitsToNumbers(tensor) {
  const d = tensor.data;
...
function utf8Length(codePoints) {
  let n = 0;
 ...
 */
export function tokenizeWithEnds(tokenizer, text) {
  const normalize = tokenizer.normalizer ? (s) => tokenizer.normalizer(s) : (s) => s;
  ...
  while (i < normChars.length) {
    ...
    const pieces = tokenizer.model([METASPACE + normChars.slice(i, j).join('')]);
...
export function sentencesFrom(chars, probs, threshold = DEFAULTS.threshold) {
  const sentences = [];
  let offset = 0;
  let idx = 0;
 ...
 */
export function joinSegments(sentences) {
  return sentences
...
export async function create({ ort, Tokenizer, readFile, executionProviders, sessionOptions }) {
  const tokenizerJson = JSON.parse(new TextDecoder().decode(await readFile('tokenizer.json')));
  const tokenizer = new Tokenizer(tokenizerJson, {});
  const session = await ort.InferenceSession.create(await readFile('model.onnx'), {
    executionProviders,
    ...sessionOptions,
  });
  const maskMeta = session.inputMetadata?.find?.((m) => m.name === 'attention_mask');
  const maskType = maskMeta?.type ?? 'float16';
  if (maskType !== 'float16' && maskType !== 'float32') throw new Error(`unexpected attention_mask type ${maskType}`);

  /** Per-token logits averaged over windows, in float16 as wtpsplit's `extract` computes them. */
  async function tokenLogits(ids, { blockSize, stride, batchSize }) {
    const n = ids.length;
    ...
    for (let b = 0; b < windows.length; b += batchSize) {
      ...
      const out = await session.run({
        input_ids: new ort.Tensor('int64', idData, [B, S]),
        attention_mask: new ort.Tensor(maskType, maskData, [B, S]),
      });
  ...
  /** wtpsplit's per-character sentence probabilities, plus the token ids that produced them. */
  async function predict(text, opts = {}) {
    const o = { ...DEFAULTS, ...opts };
    const chars = Array.from(text);
    const probs = new Float32Array(chars.length);
    if (chars.every(isPySpace)) return { chars, probs, ids: [] };
    const { ids, ends } = tokenizeWithEnds(tokenizer, text);
    if (ids.length === 0) return { chars, probs, ids };
    const logits = await tokenLogits(ids, o);
    ...
    return { chars, probs, ids };
  }

  async function split(text, opts = {}) {
    const o = { ...DEFAULTS, ...opts };
    const { chars, probs } = await predict(text, o);
    return sentencesFrom(chars, probs, o.threshold);
  }
  ...
  return {
    ...
    async punctuate(text, lang) {
      if (!info.langs.includes(lang)) throw new Error(`sat-3l-sm: unsupported language ${lang}`);
      return joinSegments(await split(text));
    },
    async release() {
